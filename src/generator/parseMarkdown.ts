import markdownit from 'markdown-it';
// @ts-ignore
import container from 'markdown-it-container';
// @ts-ignore
import deflist from 'markdown-it-deflist';
// @ts-ignore
import footnote from 'markdown-it-footnote';
// @ts-ignore
import tasklists from 'markdown-it-task-lists';
// @ts-ignore
import anchor from 'markdown-it-anchor';
// @ts-ignore
import toc from 'markdown-it-toc-done-right';
// @ts-ignore
import attrs from 'markdown-it-attrs';
// @ts-ignore
import linkAttributes from 'markdown-it-link-attributes';
// // @ts-ignore
// import plantuml from 'markdown-it-plantuml';
// // @ts-ignore
// import mermaid from 'markdown-it-mermaid';
// // import diagrams from 'markdown-it-diagrams';

import prism from './prism';

const md: any = markdownit({
    html: true,
    linkify: true,
    typographer: true,
    highlight: (str, lang) => {
        // @ts-ignore
        return `<pre class="language-${lang}"><code class="language-${lang}">${prism.highlight(
            str,
            prism.languages[lang]
        )}</code></pre>`;
        // return md.utils.escapeHtml(prism.highlight(str, prism.languages[lang]));
    },
    // highlight: (str, lang) => {
    //     return `<pre class="language-${lang}"><code class="language-${lang}">${md.utils.escapeHtml(str)}</code></pre>`;
    // },
})
    .use(container, 'warning')
    .use(deflist)
    .use(footnote)
    .use(tasklists)
    .use(anchor)
    .use(toc)
    .use(attrs)
    .use(linkAttributes, [{ pattern: /^https?:/i, attrs: { target: '_blank', rel: 'noopener' } }]);
// .use(diagrams)
// .use(mermaid)
// .use(plantuml);

// @ts-ignore
md.renderer.rules.image = function(tokens, idx, options, env, self) {
    const token = tokens[idx];
    const alt = token.content;
    const title = token.attrs[token.attrIndex('title')][1];
    const src = token.attrs[token.attrIndex('src')][1];

    return `<figure><a href="${src}"><img src="${src}" alt="${md.utils.escapeHtml(
        alt || title
    )}"/></a><figcaption>${md.utils.escapeHtml(title)}</figcaption></figure>`;
};

export default (str: string) => md.render(str);
